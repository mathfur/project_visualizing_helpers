...
module ProjectVisualizingHelpers
  class MySQLLog2CSV < Base
    ...
    COMMAND_REGEX        = /(#{COMMANDS.join('|')})/
    HEADER_LINE_REGEX    = /^ *Time +Id +Command +Argument *$/
    TIMED_LINE_REGEX     = /^(?<num>\d+)\s+(?<time>\d{2}:\d{2}:\d{2})\s+(\d+)\s+(?<command>#{COMMAND_REGEX})(\s+(?<argument>.*))?$/
    NOT_TIMED_LINE_REGEX = /^\s+(\d+)\s+(?<command>#{COMMAND_REGEX})(\s+(?<argument>.*))?$/
